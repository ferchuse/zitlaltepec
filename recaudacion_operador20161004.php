...
if($_POST['ajax']==2){
	...
	if($row=mysql_fetch_array($res)){
		...
		$res1=mysql_query("SELECT * FROM cat_cargos_operadores WHERE tipo=1");
		while($row1=mysql_fetch_array($res1)){
			$array_cargo[$row1['cve']]=$row1['monto'];
		}
		echo $row['cve']."|";
		...
		echo $array_cargo[1].'|';
		$total_reimpresion=0;
		$res1=mysql_query("SELECT * FROM tarjetas_unidad WHERE operador='".$row['operador']."' AND estatus='A' AND cve<'".$row['cve']."'");
		if(mysql_num_rows($res1)>0){
			echo '<table><tr><th>Tarjeta</th><th>Fecha Viaje</th><th>Reimpresion</th></tr>';
			while($row1=mysql_fetch_array($res1)){
				echo '<tr><td>'.$row1['cve'].'</td><td align="center">'.$row['fecha_viaje'].'</td>
				<td align="right">'.number_format($array_cargo[2],2).'
				<input type="hidden" name="reimpresionmonto['.$row1['cve'].'" value="'.$array_cargo[2].'">
				<input type="hidden" name="reimpresionfecha['.$row1['cve'].'" value="'.$row1['fecha_viaje'].'"></td></tr>';
				$total_reimpresion+=$array_cargo[2];
			}
			echo '</table>';
		}
		echo '|'.$total_reimpresion.'|';
		echo round($total_reimpresion+$array_cargo[1],2).'|';
...
if($_POST['cmd']==12){
	...
	if($continuar){
		...
		$impresion= '<script>alert("La unidad ya tiene mutualidad en el dia");window.close();</script>';
	}
	echo '<html><body>'.$impresion.'</body></html>';
	echo '<script>setTimeout("window.close()",4000);</script>';
	exit();
...
if($_POST['cmd']==2){
	...
	if($continuar){
		...
		echo '<script>alert("La unidad ya tiene mutualidad en el dia");</script>';
	}
	$_POST['cmd']=0;
}
