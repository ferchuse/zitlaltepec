...
if($_POST['ajax']==1) {
		...
		$select= " SELECT * FROM taquillas_sencillos WHERE 1 ";
		...
		echo '<tr><td bgcolor="#E9F2F8" colspan="6">'.mysql_num_rows($res).' Registro(s)</td></tr>';
			echo '<tr bgcolor="#E9F2F8"><th>&nbsp;</th><th>Numero</th><th>Nombre</th></tr>';
			while($row=mysql_fetch_array($res)) {
				rowb();
				echo '<td align="center"><a href="#" onClick="atcr(\'taquillas_sencillos.php\',\'\',1,\''.$row['cve'].'\')"><img src="images/modificar.gif" border="0" title="Editar"></a></td>';
				echo '<td align="left">'.utf8_encode($row['cve']).'</td>';
...
if ($_POST['cmd']==2) {
	if($_POST['reg']) {
		$res = mysql_query("SELECT * FROM taquillas_sencillos WHERE cve='".$_POST['reg']."'");
		...
		$update = " UPDATE taquillas_sencillos 
		...
		$insert = " INSERT taquillas_sencillos 
					SET nombre='".$_POST['nombre']."',tipo_impresora='".$_POST['tipo_impresora']."',nombre_impresora='".addslashes($_POST['nombre_impresora'])."'";
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		$select=" SELECT * FROM taquillas_sencillos WHERE cve='".$_POST['reg']."' ";
		$res=mysql_query($select);
		$row=mysql_fetch_array($res);
		//Menu
		echo '<table>';
		echo '
			<tr>';
		if(nivelUsuario()>1)
			echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'taquillas_sencillos.php\',\'\',\'2\',\''.$row['cve'].'\');"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
		echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'taquillas_sencillos.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>
			</tr>';
		echo '</table>';
		echo '<br>';
		...
		//Formulario 
		echo '<table>';
		echo '<tr><td class="tableEnc">Edicion Taquillas sin Guia</td></tr>';
		echo '</table>';
		...
		echo '><th>Tipo Impresora</th><td><select name="tipo_impresora" id="tipo_impresora">
		<option value="0">LPT</option>
		<option value="1"';
		if($row['tipo_impresora'] == 1) echo ' selected';
		echo '>USB</option></select></td></tr>';
		echo '<tr';
		if($_POST['cveusuario']!=1) echo ' style="display:none;"';
		echo '><th>Nombre Impresora</th><td><input type="text" name="nombre_impresora" id="nombre_impresora" class="textField" size="100" value="'.$row['nombre_impresora'].'"></td></tr>';
		echo '</table>';
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<td><a href="#" onClick="atcr(\'taquillas_sencillos.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>
			 </tr>';
		echo '</table>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","taquillas_sencillos.php",true);
