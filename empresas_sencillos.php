...
if($_POST['ajax']==1) {
		...
		$select= " SELECT * FROM empresas_sencillos WHERE 1 ";
...
if ($_POST['cmd']==2) {
	if($_POST['reg']) {
		$res = mysql_query("SELECT * FROM empresas_sencillos WHERE cve='".$_POST['reg']."'");
		...
		$update = " UPDATE empresas_sencillos 
		...
		$insert = " INSERT empresas_sencillos 
					SET nombre='".$_POST['nombre']."',logo='".$_POST['logo']."'";
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		$select=" SELECT * FROM empresas_sencillos WHERE cve='".$_POST['reg']."' ";
		$res=mysql_query($select);
		$row=mysql_fetch_array($res);
		//Menu
		echo '<table>';
		echo '
			<tr>';
		if(nivelUsuario()>1)
			echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'empresas_sencillos.php\',\'\',\'2\',\''.$row['cve'].'\');"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
		echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'empresas_sencillos.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>
			</tr>';
		echo '</table>';
		echo '<br>';
		...
		//Formulario 
		echo '<table>';
		echo '<tr><td class="tableEnc">Edicion Empresas Taquilla sin Guia</td></tr>';
		echo '</table>';
		...
		echo '<tr><th>Logo</th><td><input type="text" name="logo" id="logo" class="textField" size="50" value="'.$row['logo'].'"></td></tr>';
		echo '</table>';
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<td><a href="#" onClick="atcr(\'empresas_sencillos.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>
			 </tr>';
		echo '</table>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","empresas_sencillos.php",true);
