...
$array_beneficiario = array(1=>'Gastos de Administracion', 2=>'Seguro Interno');

if($_POST['ajax']==1){
	$nivelUsuario = nivelUsuario();
	echo '<table width="100%" border="0" cellpadding="4" cellspacing="1" class="">';
	echo '<tr bgcolor="#E9F2F8"><th>&nbsp;</th>';
	echo '<th>Folio</th><th>Fecha Creacion</th>';
	echo '<th>Fecha Aplicacion</th><th>Operador</th>
	<th>Unidad</th><th>Beneficiario</th><th>Monto</th><th>Concepto</th><th>Usuario</th>
	</tr>'; 
	$x=0;
	$filtroruta="";
	$select="SELECT a.* FROM devolucion_fianza a ";
	...
	while($row=mysql_fetch_array($res)){
		...
		if($row['estatus']!='C'){
			echo '<a href="#" onClick="atcr(\'devolucion_fianza.php\',\'_blank\',10,\''.$row['cve'].'\')"><img src="images/b_print.png" border="0" title="Imprimir"></a>';
			if($nivelUsuario > 2)
				echo '<a href="#" onClick="atcr(\'devolucion_fianza.php\',\'\',3,\''.$row['cve'].'\')"><img src="images/validono.gif" border="0" title="Cancelar"></a>';
		}
...
if($_POST['ajax']==2){
	if($_POST['operador'] > 0)
		echo saldo_operador_fianza($_POST['operador']);
	exit();
}
...
if($_POST['cmd']==10){
	$res = mysql_query("SELECT * FROM devolucion_fianza WHERE cve='".$_POST['reg']."'");
	$row = mysql_fetch_array($res);
	$texto =chr(27)."@".'|';
	$texto.=chr(27).'!'.chr(40)."  TRASPASO DE FIANZA";
	$texto.='||';
	...
	$texto.='|';
	$texto.=chr(27).'!'.chr(10)."BENEFICIARIO: ".$array_beneficiario[$row['beneficiario']];
	$texto.='|';
...
if($_POST['cmd']==3){
	mysql_query("UPDATE devolucion_fianza SET estatus='C',fechacan='".fechaLocal()." ".horaLocal()."',usucan='".$_POST['cveusuario']."' WHERE cve='".$_POST['reg']."'");
	$_POST['cmd']=0;
}
...
if($_POST['cmd']==2){
	...
	if($continuar){
		mysql_query("INSERT devolucion_fianza SET 
			fecha='".fechaLocal()."',
			fecha_aplicacion='".$_POST['fecha']."',operador='".$_POST['operador']."',hora='".horaLocal()."',unidad='".$_POST['unidad']."',
			usuario='".$_POST['cveusuario']."',estatus='A',obs='".$_POST['obs']."',empresa='".$array_empconductor[$_POST['operador']]."',
			monto='".$_POST['monto']."',beneficiario='".$_POST['beneficiario']."',saldo='".$_POST['saldo']."'") or die(mysql_error());
		$cverecaudacion=mysql_insert_id();
		$res = mysql_query("SELECT * FROM devolucion_fianza WHERE cve='".$cverecaudacion."'");
		$row = mysql_fetch_array($res);
		$texto =chr(27)."@".'|';
		$texto.=chr(27).'!'.chr(40)."  TRASPASO DE FIANZA";
		$texto.='||';
...
if($_POST['cmd']==1){
	...
	if($nivelUsuario>1)
		echo '<td><a href="#" onClick="
				if(document.forma.operador.value==\'\'){
					alert(\'Necesita seleccionar el operador\');
				}
				else if(document.forma.beneficiario.value==\'\')
					alert(\'Necesita seleccionar al beneficiario\');
				else if((document.forma.saldo.value/1) < (document.forma.monto.value/1))
					alert(\'No puede transferir mas del saldo\');
				else{
					atcr(\'devolucion_fianza.php\',\'\',\'2\',\'0\');
				}
				"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
		echo '<td><a href="#" onClick="atcr(\'devolucion_fianza.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>';
	
	echo '</tr>';
	echo '</table>';
	echo '<br>';
	echo '<table>';
	echo '<tr><td class="tableEnc">Traspaso de Fianzaz</td></tr>';
	echo '</table>';
	echo '<table>';
	echo '<tr><th align="left">Fecha Aplicacion</th><td><input type="text" class="readOnly" name="fecha" id="fecha" value="'.fechaLocal().'" size="15" readOnly>';
	if($nivelUsuario>2){
		echo '&nbsp;<a href="#" onClick="displayCalendar(document.forms[0].fecha,\'yyyy-mm-dd\',this,true)"><img src="images/calendario.gif" border="0"></a>';
	}
	echo '</td></tr>';
	echo '<tr><th align="left">Operador</th><td><select name="operador" id="operador" onChange="traerSaldo()" class="textField"><option value="">Seleccione</option>';
	...
	echo '</select></td></tr>';
	echo '<tr><th align="left">Saldo</th><td><input type="text" name="saldo" id="saldo" size="10" value="'.$row['saldo'].'" class="readOnly" readOnly></td></tr>';
	echo '<tr><th align="left">Beneficiario</th><td><select name="beneficiario" id="beneficiario" class="textField"><option value="">Seleccione</option>';
	foreach($array_beneficiario as $k=>$v){
		echo '<option value="'.$k.'"';
		if($row['beneficiario']==$k) echo ' selected';
		echo '>'.$v.'</option>';
	}
	echo '</select></td></tr>';
	
	echo '<tr><th align="left">Unidad</th><td><input type="text" name="unidad" id="unidad" size="10" value="'.$row['unidad'].'" class="textField"></td></tr>';
	echo '<tr><th align="left">Monto</th><td><input type="text" name="monto" id="monto" size="10" value="'.$row['monto'].'" class="textField"></td></tr>';
	echo '<tr><th align="left">Concepto</th><td><textarea cols="30" rows="3" id="obs" name="obs" class="textField">'.$row['obs'].'</textarea></td></tr>';
	echo '</table>';
	...
	echo '<script>
				function traerSaldo(){
					...
					if (objeto1.readyState != 0) {
						...
						objeto1.open("POST","devolucion_fianza.php",true);
						objeto1.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
						objeto1.send("ajax=2&operador="+document.forma.operador.value);
						objeto1.onreadystatechange = function(){
							if (objeto1.readyState==4){
								document.forma.saldo.value=objeto1.responseText;
							}
						}
					}
				}
...
if($_POST['cmd']==0){
	...
	echo '<tr>
			...
			<td><a href="#" onClick="atcr(\'devolucion_fianza.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>
			<!--<td><a href="#" onclick="atcr(\'devolucion_fianza.php\',\'_blank\',\'excel\',0);"><img src="images/b_print.png" border="0">&nbsp;&nbsp;Excel</a>&nbsp;&nbsp;</td>-->
		</tr>';
	echo '</table>';
	echo '<table>';
	$dia=date("w");
