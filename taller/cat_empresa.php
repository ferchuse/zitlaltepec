...
if($_POST['ajax']==1){
	...
	$select= " SELECT * FROM cat_empresa WHERE 1";
...
//	else if($_POST['apodo']!=0) $select .= " AND id = '".$_POST['apodo']."'";
	...
	while($row=mysql_fetch_array($res)){
		rowb();
		echo'<td align="center"><a href="#" onClick="atcr(\'cat_empresa.php\',\'\',\'1\','.$row['cve'].')"><img src="images/modificar.gif" border="0" title="Editar"></a></td>';
...
exit();
 ...
 top($_SESSION);
  if($_POST['cmd']==2){
	  $str_error = '';
	if ($_POST['reg']>0){
        $select=" SELECT * FROM cat_empresa WHERE cve='".$_POST['reg']."' ";
	   ...
	   if($Usuario['nombre']!=$_POST['nombre']){
		...
		$sSQL="update cat_empresa
				SET nombre='" . $_POST['nombre'] . "' where cve='".$_POST['reg']."'";
		mysql_query($sSQL);
	}else{
			$sSQL="INSERT cat_empresa
...
if($_POST['cmd']==1){
    ...
    echo'
	    <a href="#" onClick="atcr(\'cat_empresa.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver </a>
		<a href="#" onClick="atcr(\'cat_empresa.php\',\'\',\'2\',\''.$provedor['cve'].'\');">&nbsp;<img src="images/guardar.gif" border="0"> &nbsp;Guardar</a></br></br>
	...
	echo '
	 ...
	 function validar(reg)
		...
		function validar_tel(reg)
	   {
			   ...
			   {
                       objeto.open("POST","cat_empresa.php",true);
...
//			$apodos .= '<option value="' . $row['id'] . '">' . $row['apodo'] . '</option>';
		...
		echo '<tr>
				...
				<a href="#" onClick="atcr(\'cat_empresa.php\',\'\',\'1\',\'0\');" id="nuevo" name="nuevo" ><img src="images/nuevo.gif"></a>Nuevo</td>
			 </tr>';
		echo '</table>';
		echo '<table>';
...
//		echo '<tr><td>Fecha Final </td><td><input type="text" name="fecha_fin" id="fecha_fin" value="'.fechaLocal().'" class="textField" size="12">&nbsp;<a href="#" onClick="displayCalendar(document.forms[0].fecha_fin,\'yyyy-mm-dd\',this,true)"><img src="images/calendario.gif" border="0"></a></td></tr>';
		echo '<tr><td>Empresa</td><td><input type="text" class="textField" size="" name="empresa" id="empresa"></td>';
