...
if($_POST['ajax']==1){
	...
	$resultados = array();
	$res = mysql_query("SELECT a.hora, b.folio, c.clave, d.no_eco, d.imei, b.estatus, b.fecha, b.hora as hora_tarjeta FROM rolesfenix a LEFT JOIN viajesfenix b ON a.cve = b.turno AND b.estatus != 'C' LEFT JOIN conductoresfenix c ON c.cve = b.conductor LEFT JOIN parquefenix d ON d.cve = b.unidad WHERE a.plaza=1 AND a.fecha='".$_POST['fecha']."' ORDER BY a.hora") or die(mysql_error());
	while($row=mysql_fetch_array($res)){
		$resultados[] = $row;
	}
	...
	for($z=0;$z<count($resultados);$z++){
		...
		if($mostrar){
			...
			if(count($resultadovueltas) == 0) $resultadovueltas = $array_resultadopuntos[0];
				...
				$horapropuesta='';
				foreach($array_puntos[0] as $cve => $datos){
					$duracion=$datos[0];
					$puntoinicio = $i;
					$encontrado = false;
					while($i<$puntos){
			...
			$resultadoida[$row['no_eco']][$horar] = $row['hora'];
		}
	}
	...
	$select= " SELECT * FROM geocercas WHERE base = 1 AND ruta = '$ruta' AND orden > 0 ORDER BY orden DESC";
	$array_puntos2 = array();
	$res = mysql_query($select);
	$cambio = false;
	while($row = mysql_fetch_array($res)){
		$array_puntos2[$row['direccion']][$row['cvebase']] = array($row['duracion'], $row['codigo']);
	}
	foreach($array_puntos2[1] as $k=>$v){
		echo '<th colspan="3">'.$k.''.$v[1].'</th>';
	}
	echo '<th rowspan="2">Tiempo Regreso</th><th rowspan="2">&nbsp;&nbsp;</th>';
	echo '</tr>';
	echo '<tr bgcolor="#E9F2F8">';
	foreach($array_puntos2[1] as $v){
		echo '<th>H.P.</th><th>H.R.</th><th>DIF.</th>';
	}
	echo '</tr>';
	$resultadoregreso=array();
	for($z=0;$z<count($resultados);$z++){
		$row = $resultados[$z];
		$row2 = obtener_siguiente_rol($resultados, $z, $row['no_eco']);
		$row2['hora'] = ($row2['primerahora'] == '') ? $row2['hora'] : $row2['primerahora'];
		if($row2['hora'] == '') $row2['hora'] = '23:59:59';
		$mostrar = 1;
		if(($_POST['salida'] != '' && $_POST['salida'] != $row1['folio']) || $row['folio'] == ""){
			$mostrar = 0;
		...
		if($mostrar){
			...
			//$row['hora'] = $resultadoida[$row['no_eco']][$row['hora']];
			$row['hora'] = $resultados[$z]['ultimahora'];
			$cvepuntos = "";
			...
			if(count($resultadovueltas) == 0) $resultadovueltas = $array_resultadopuntos[0];
				...
				$puntos_encontrados = 0;
				$arreglohtml=array();
				foreach($array_puntos[1] as $cve => $datos){
					$duracion = $datos[0];
					$puntoinicio = $i;
					$encontrado = false;
					while($i<$puntos){
						if($resultadovueltas[$i]['idpunto'] == $cve){
							...
							$html2 = '<td style="border-left: solid 5px #000" align="center">'.$horapropuesta.'</td>';
							$limiteini = date( "H:i:s" , strtotime ( "-2 minute" , strtotime($horapropuesta) ) );
							$limitefin = date( "H:i:s" , strtotime ( "+ 2 minute" , strtotime($horapropuesta) ) );

							$diferencia=$diferencia = restahoras(substr($resultadovueltas[$i]['horapunto'],-8),$horapropuesta,0);
							$html2 .= '<td align="center"><div width="100%" style="background-color: #FFFF00;">'.substr($resultadovueltas[$i]['horapunto'],-8).'</div></td>';

							if(substr($resultadovueltas[$i]['horapunto'],-8) <= $limitefin && substr($resultadovueltas[$i]['horapunto'],-8) >= $limiteini)
								$html2.= '<td style="border-right: solid 5px #000" align="center"><font color="GREEN">'.$diferencia.'</font></td>';
							elseif(substr($resultadovueltas[$i]['horapunto'],-8)<$limiteini)
								$html2.= '<td style="border-right: solid 5px #000" align="center"><font color="BLUE">'.$diferencia.'</font></td>';
							else
								$html2.= '<td style="border-right: solid 5px #000" align="center"><font color="RED">'.$diferencia.'</font></td>';
					...
					if(!$encontrado){
						...
						if($cve == $primerpunto)
							...
							$horapropuesta = date( "H:i:s" , strtotime ( "+ ".$duracion." minute" , strtotime($horapropuesta) ) );
						$html2 = '<td style="border-left: solid 5px #000" align="center">'.$horapropuesta.'</td>';
						$html2 .= '<td>&nbsp;</td><td style="border-right: solid 5px #000">&nbsp;</td>';
						$arreglohtml[] = $html2;
					}
				}
				if($puntos_encontrados >= 0){
					rsort($arreglohtml);
					foreach($arreglohtml as $html) echo $html;
				}
				echo '<td>'.$tiempo.'</td>';
			//}
			echo '</tr>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","recorridoffcc4.php",true);
