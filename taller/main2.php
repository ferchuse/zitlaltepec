...
$array_base=array(1=>'road', 2=>'GPS3');
...
/*$array_modulos=array(1=>"CATALOGOS",2=>'OPERACIONES',3=>'MESAS DE CONTROL',99=>"ADMINISTRACION", 100 => 'TELEFONIA CON GPS',101 => 'AUTOMOVIL',102 => 'TRAKING',103=>'REPORTES GPS',105=>"NUEVA INTERFAZ",104=>'TAREAS');*/
//

$array_modulos=array(1=>"CATALOGOS",2=>'OPERACIONES',3=>'MESAS DE CONTROL',99=>"ADMINISTRACION", 100 => 'TELEFONIA CON GPS',-1002 => 'TRAKING',-103=>'REPORTES GPS', 105=>"NUEVA INTERFAZ",-1004=>'TAREAS');
...
function top($SESSION,$enter=0){
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabla_encabezado">
	  <tr>
		...
		<td align="right" valign="top" bgcolor="#B6C5EE">
			&nbsp;
	    </td>
		</tr>
  </table>
	...
	if($_POST['plazausuario']>0){
		foreach($array_modulos as $k=>$v){
			...
			elseif($SESSION['TipoUsuario']==1){
				$rs=mysql_query("SELECT * FROM menu WHERE modulo='$k' AND menupadre=0 AND cve!=52 ORDER BY orden");
			}
			else{
				$rs=mysql_query("SELECT a.* FROM menu as a INNER JOIN usuario_accesos as b ON (b.menu=a.cve AND b.usuario='".$_POST['cveusuario']."' AND b.plaza='".$_POST['plazausuario']."' AND b.acceso>0) WHERE a.modulo='$k' AND a.menupadre=0 AND a.cve NOt IN (1,52) ORDER BY a.orden");
			}
			if(mysql_num_rows($rs)>0){
				...
				while($ro=mysql_fetch_array($rs)) {
					...
					if(mysql_num_rows($re1) > 0){
						...
						if($_POST['plazausuario']!=10 and $ro['cve']==55){
							echo ",";
						}else 
						{
		...
		if($_POST['cveusuario']==1){
			echo"['', 'Catalogo de Recordatorios', 'javascript: document.forma.cvemenu.value=100;atcr(\'cat_recordatorios.php\',\'\',\'\',\'\');','',''],
				['', 'Recordatorios', 'javascript: document.forma.cvemenu.value=101;atcr(\'recordatorios.php\',\'\',\'\',\'\');','',''],";
		}
		echo "
