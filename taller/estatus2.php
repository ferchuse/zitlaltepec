...
if($_POST['ajax']==1){
	...
	$select= " SELECT * FROM estatus WHERE 1";
	...
	while($row=mysql_fetch_array($res)){
		rowb();
		echo'<td align="center"><a href="#" onClick="atcr(\'estatus2.php\',\'\',\'1\','.$row['cve'].')">'.$imgeditar.'</a></td>';
...
exit();
 ...
 top($_SESSION);
  if($_POST['cmd']==2){
	  ...
	  $str_error = '';
	if ($_POST['reg']>0){
        $select=" SELECT * FROM estatus WHERE cve='".$_POST['reg']."' ";
       $res=mysql_query($select);
       $row=mysql_fetch_array($res);
	   if($Usuario['nombre']!=$_POST['nombre']){
			mysql_query("INSERT historial SET tabla='estatus',registro='".$_POST['reg']."',fecha='".fechaLocal()." ".horaLocal()."',
...
if($_POST['cmd']==1){
    ...
    echo'
    	<a href="#" onClick="atcr(\'estatus2.php\',\'\',\'2\',\''.$row['cve'].'\');">&nbsp;'.$imgguardar.'&nbsp;Guardar</a>&nbsp;&nbsp;
	    <a href="#" onClick="atcr(\'estatus2.php\',\'\',\'0\',\'0\');">'.$imgvolver.'&nbsp;Volver </a>
		</br></br>
		<table>
		<tr>';
 ...
 if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<a href="#" onClick="atcr(\'estatus2.php\',\'\',\'1\',\'0\');" id="nuevo" name="nuevo" ><img src="images/nuevo.gif"></a>Nuevo</td>
			 </tr>';
		echo '</table>';
		echo '<table>';
...
<Script language="javascript">
	function buscarRegistros(){
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","estatus2.php",true);
