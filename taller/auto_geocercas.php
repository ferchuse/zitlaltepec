...
if($usuarioempresa != '')
	$rsMotivo=mysql_query("SELECT * FROM cat_rutas WHERE usuario=$usuarioempresa ORDER BY nombre");
else
...
if($_POST['ajax']==1){
	...
	$select= " SELECT * FROM geocercas_gps WHERE 1";
	...
	echo '<table width="100%" border="0" cellpadding="4" cellspacing="1" class="" id="tabla1">
	      <tr bgcolor="#E9F2F8">
		  <th width="50"></th><th>Nombre</th><th>Ruta</th><th>Orden</th><th>Empresa</th>';
		  if($_POST['cveusuario']==1) echo '<th>Borrar</th>';
	echo '
		  </tr>';
	while($row=mysql_fetch_array($res)){
		rowb();
		echo'<td align="center"><a href="#" onClick="atcr(\'auto_geocercas.php\',\'\',\'1\','.$row['cve'].')">'.$imgeditar.'</a></td>';
//	echo'<td align="left"></td>';
		...
		if($_POST['cveusuario']==1)
			echo'<td align="center"><a href="#" onClick="if(confirm(\'Esta seguro de eliminar el registro?\'))atcr(\'auto_geocercas.php\',\'\',\'3\','.$row['cve'].')">'.$imgborrar.'</a></td>';
		echo'</tr>';
	}
 ...
 top($_SESSION);
   if($_POST['cmd']==3){
   	mysql_select_db('road_gps_sky_media');
	$sSQL="delete from geocercas_gps where cve='".$_POST['reg']."'";
	mysql_query($sSQL);
	mysql_select_db('road_gps');
   	$_POST['cmd']=0;
   }
...
if($_POST['cmd']==1){
	mysql_select_db('road_gps_sky_media');
	    $select=" SELECT * FROM geocercas_gps WHERE cve='".$_POST['reg']."' ";
	    $rsprovedor=mysql_query($select);
	    $provedor=mysql_fetch_array($rsprovedor);
	

    echo'
	    <a href="#" onClick="atcr(\'auto_geocercas.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver </a>
		<a href="#" onClick="atcr(\'auto_geocercas.php\',\'\',\'2\',\''.$provedor['cve'].'\');">&nbsp;<img src="images/guardar.gif" border="0"> &nbsp;Guardar</a></br></br>
		<table>
		<tr>';
...
<Script language="javascript">
	function buscarRegistros(){
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","auto_geocercas.php",true);
