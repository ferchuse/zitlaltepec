...
*/
$rsMotivo=mysql_query("SELECT * FROM cat_tecnologias ORDER BY nombre");
while($Motivo=mysql_fetch_array($rsMotivo)){
	$array_tecnologia[$Motivo['cve']]=$Motivo['nombre'];
}
$rsMotivo=mysql_query("SELECT * FROM cat_actividad ORDER BY nombre");
while($Motivo=mysql_fetch_array($rsMotivo)){
	$array_actividad[$Motivo['cve']]=$Motivo['nombre'];
}
if($_POST['ajax']==1){


	//	$select= " SELECT * FROM sms_comandos WHERE plaza = '{$_SESSION['plaza_seleccionada']}'";
	$select= " SELECT * FROM tipo_mantenimiento WHERE 1";
...
//	else if($_POST['apodo']!=0) $select .= " AND id = '".$_POST['apodo']."'";
	...
	echo '<table width="100%" border="0" cellpadding="4" cellspacing="1" class="" id="tabla1">
	      <tr bgcolor="#E9F2F8">
		  <th width="50"></th><th>Nombre</th><th>Km</th><th>Tecnologia</th><th>Actividad</th>
		  </tr>';
	while($row=mysql_fetch_array($res)){
		rowb();
		echo'<td align="center"><a href="#" onClick="atcr(\'tipo_mantenimiento.php\',\'\',\'1\','.$row['cve'].')"><img src="images/modificar.gif" border="0" title="Editar"></a></td>';
//	echo'<td align="left"></td>';
		echo'<td align="">'.$row['nombre'].'</td>';
		echo'<td align="center">'.$row['km'].'</td>';
		echo'<td align="center">'.$array_tecnologia[$row['tecnologia']].'</td>';
		echo'<td align="center">'.$array_actividad[$row['actividad']].'</td>';
...
exit();
 ...
 top($_SESSION);
  if($_POST['cmd']==2){
	  $str_error = '';
	if ($_POST['reg']>0){
        $select=" SELECT * FROM tipo_mantenimiento WHERE cve='".$_POST['reg']."' ";
...
/*		if($Usuario['cambio_aceite']!=$_POST['cambio_aceite']){
		...
		$sSQL="update tipo_mantenimiento
				SET nombre='" . $_POST['nombre'] . "',km='" . $_POST['km'] . "',tecnologia='" . $_POST['tecnologia'] . "',actividad='" . $_POST['actividad'] . "' where cve='".$_POST['reg']."'";
		mysql_query($sSQL);
	}else{
			$sSQL="INSERT tipo_mantenimiento
					SET fecha='" .fechaLocal(). "',hora='" .horaLocal(). "',nombre='" . $_POST['nombre'] . "',
					km='" . $_POST['km'] . "',tecnologia='" . $_POST['tecnologia'] . "',actividad='" . $_POST['actividad'] . "',plaza='".$_POST['plazausuario']."',usuario='".$_POST['cveusuario']."'";
			mysql_query($sSQL);
...
if($_POST['cmd']==1){
    ...
    echo'
	    <a href="#" onClick="atcr(\'tipo_mantenimiento.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver </a>
		<a href="#" onClick="atcr(\'tipo_mantenimiento.php\',\'\',\'2\',\''.$provedor['cve'].'\');">&nbsp;<img src="images/guardar.gif" border="0"> &nbsp;Guardar</a></br></br>
...
/*		echo'<tr><td>Empresa</td><td><select name="empresa" id="empresa" class="textField"><option value="">---Seleccione---</option>';
		...
		echo'<tr><td>Tecnologia</td><td><select name="tecnologia" id="tecnologia" class="textField"><option value="">---Seleccione---</option>';
		foreach ($array_tecnologia as $k=>$v) { 
	    echo '<option value="'.$k.'"';if($provedor['tecnologia']==$k){echo'selected';}echo'>'.$v.'</option>';
			}
		echo '</select></td></tr>';
		echo'<tr><td>Actividad</td><td><select name="actividad" id="actividad" class="textField"><option value="">---Seleccione---</option>';
		foreach ($array_actividad as $k=>$v) { 
	    echo '<option value="'.$k.'"';if($provedor['actividad']==$k){echo'selected';}echo'>'.$v.'</option>';
			}
		echo '</select></td></tr>';
	...
	echo '
	 ...
	 function validar(reg)
	   ...
	   else if(document.getElementById("comando").value=="0"  )
	   {
		   ...
		   atcr("tipo_mantenimiento.php","",2,reg);
       }
		}
		function validar_tel(reg)
	   {
			   ...
			   {
                       objeto.open("POST","tipo_mantenimiento.php",true);
...
//			$apodos .= '<option value="' . $row['id'] . '">' . $row['apodo'] . '</option>';
		...
		echo '<tr>
				...
				<a href="#" onClick="atcr(\'tipo_mantenimiento.php\',\'\',\'1\',\'0\');" id="nuevo" name="nuevo" ><img src="images/nuevo.gif"></a>Nuevo</td>
			 </tr>';
		echo '</table>';
		echo '<table>';
