...
if($_POST['ajax']==1){
	...
	$select= " SELECT * FROM tareas_mantenimiento WHERE 1";
...
//	else if($_POST['apodo']!=0) $select .= " AND id = '".$_POST['apodo']."'";
	...
	echo '<table width="100%" border="0" cellpadding="4" cellspacing="1" class="" id="tabla1">
	      <tr bgcolor="#E9F2F8">
		  <th width="50"></th><th>N°</th><th>Tarea</th><th>Cambio de Aceite</th><th>Camioneta</th><th>Autobus</th>
		  </tr>';
	while($row=mysql_fetch_array($res)){
		rowb();
		echo'<td align="center"><a href="#" onClick="atcr(\'tareas_mantenimiento.php\',\'\',\'1\','.$row['cve'].')"><img src="images/modificar.gif" border="0" title="Editar"></a></td>';
//	echo'<td align="left"></td>';
		echo'<td align="center">'.$row['cve'].'</td>';
		echo'<td align="left">'.$row['tarea'].'</td>';
		echo'<td align="left">'.$row['cambio_aceite'].'</td>';
		echo'<td align="center">'.$row['km_camioneta'].'</td>';
		echo'<td align="center">'.$row['km_autobus'].'</td>';
...
exit();
 ...
 top($_SESSION);
  if($_POST['cmd']==2){
	  $str_error = '';
	if ($_POST['reg']>0){
        $select=" SELECT * FROM tareas_mantenimiento WHERE cve='".$_POST['reg']."' ";
       $rsprovedor=mysql_db_query($base,$select);
       $Usuario=mysql_fetch_array($rsprovedor);
	   if($Usuario['tarea']!=$_POST['tarea']){
			mysql_query("INSERT historial SET menu='".$_POST['cvemenu']."',cveaux='".$_POST['reg']."',fecha='".fechaLocal()." ".horaLocal()."',
			dato='Tarea',nuevo='".$_POST['tarea']."',anterior='".$Usuario['tarea']."',arreglo='',usuario='".$_POST['cveusuario']."'");
		}
...
/*		if($Usuario['dispositivo']!=$_POST['dispositivo']){
		...
		$sSQL="update tareas_mantenimiento
				SET tarea='" . $_POST['tarea'] . "',cambio_aceite='" . $_POST['cambio_aceite'] . "',km_autobus='" . $_POST['km_autobus'] . "',km_camioneta='" . $_POST['km_camioneta'] . "'
				 where cve='".$_POST['reg']."'";
		mysql_query($sSQL);
	}else{
			$sSQL="INSERT tareas_mantenimiento
					SET fecha='" .fechaLocal(). "',hora='" .horaLocal(). "',tarea='" . $_POST['tarea'] . "',
					cambio_aceite='" . $_POST['cambio_aceite'] . "',
					km_autobus='" . $_POST['km_autobus'] . "',km_camioneta='" . $_POST['km_camioneta'] . "',plaza='".$_POST['plazausuario']."'";
			mysql_query($sSQL);
...
if($_POST['cmd']==1){
    ...
    echo'
	    <a href="#" onClick="atcr(\'tareas_mantenimiento.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver </a>
		<a href="#" onClick="atcr(\'tareas_mantenimiento.php\',\'\',\'2\',\''.$provedor['cve'].'\');">&nbsp;<img src="images/guardar.gif" border="0"> &nbsp;Guardar</a></br></br>
...
/*		echo'<tr><td>Empresa</td><td><select name="empresa" id="empresa" class="textField"><option value="">---Seleccione---</option>';
		...
		echo '</select></td></tr>';*/
		echo'<tr>';
		echo'<tr><td>Tarea</td><td><input type="text" name="tarea" id="tarea" value="' . $provedor['tarea'] . '"/></td></tr>';
		echo'<tr><td>Cambio de Aceite</td><td><input type="text" name="cambio_aceite" id="cambio_aceite" value="' . $provedor['cambio_aceite'] . '"/></td></tr>';
		echo'<tr><td>Camioneta</td><td><input type="text" name="km_camioneta" id="km_camioneta" value="' . $provedor['km_camioneta'] . '"/> Km</td></tr>';
		echo'<tr><td>Autobus</td><td><input type="text" name="km_autobus" id="km_autobus" value="' . $provedor['km_autobus'] . '"/>Km</td></tr>';
		/*echo'<tr><td>Dispositivo</td><td><select name="dispositivo" id="dispositivo" class="textField"><option value="">---Seleccione---</option>';
	...
	echo '
	 ...
	 function validar(reg)
	   ...
	   else if(document.getElementById("comando").value=="0"  )
	   {
		   ...
		   atcr("tareas_mantenimiento.php","",2,reg);
       }
		}
		function validar_tel(reg)
	   {
			   ...
			   {
                       objeto.open("POST","tareas_mantenimiento.php",true);
...
//			$apodos .= '<option value="' . $row['id'] . '">' . $row['apodo'] . '</option>';
		...
		echo '<tr>
				...
				<a href="#" onClick="atcr(\'tareas_mantenimiento.php\',\'\',\'1\',\'0\');" id="nuevo" name="nuevo" ><img src="images/nuevo.gif"></a>Nuevo</td>
			 </tr>';
		echo '</table>';
		echo '<table>';
...
//		echo '<tr><td>Fecha Final </td><td><input type="text" name="fecha_fin" id="fecha_fin" value="'.fechaLocal().'" class="textField" size="12">&nbsp;<a href="#" onClick="displayCalendar(document.forms[0].fecha_fin,\'yyyy-mm-dd\',this,true)"><img src="images/calendario.gif" border="0"></a></td></tr>';
		echo '<tr><td>Folio</td><td><input type="text" class="textField" size="" name="numero" id="numero"></td>';
