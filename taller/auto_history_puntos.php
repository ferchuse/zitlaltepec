...
$select= " SELECT * FROM puntos ORDER BY orden";
$array_puntos = array();
...
mysql_select_db($base);
	...
	while($row1 = mysql_fetch_assoc($res1)){
		...
		$res = mysql_query("SELECT * FROM gps_objects_history 
			WHERE fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND imei = '".$row1['imei']."' AND punto IN ($cvepuntos)
			AND punto > 0 AND distancia <= 4
...
<Script language="javascript">
	function buscarRegistros(){
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","auto_history_puntos.php",true);
