...
$array_cargo = array(1=>'Administracion', 2=>'Seguro Interno', 3=>'Mutualidad', 4=>'Prorrata',5=>'Seguridad', 6=>'Fianza',7=>'Tag',8=>'Bases');
...
if($_POST['cmd']==100){
	...
	$rssalida=mysql_query("SELECT * FROM recibos_salidas WHERE cve='".$_POST['reg']."'");
	...
	$pdf->Cell(95,4,'Estatus: '.$array_estatus2[$Salida['estatus2']],0,0,'R');
	$pdf->Ln();
...
if($_POST['cmd']==102){
	...
	$rssalida=mysql_query("SELECT * FROM recibos_salidas WHERE cve='".$_POST['sel'][$i]."'");
	$Salida=mysql_fetch_array($rssalida);
	$pdf->AddPage();
	$pdf->SetFont('Arial','B',18);
	$pdf->Cell(190,10,'Grupo Zitlaltepec',0,0,'C');
...
if($_POST['cmd']==101){
	...
	class FPDF2 extends PDF_MC_Table {
		function Header(){
			...
			$this->SetY(23);
			$this->MultiCell(275,5,'Listado de Recibos de Salida',0,'C');
			$this->Ln();
	...
	$pdf->SetAligns(array('C','C','C','C','L','L','R','L','L'));
	$suma=0;
	for($i=0;$i<count($_POST['sel']);$i++){
		$res=mysql_query("SELECT * FROM recibos_salidas WHERE cve='".$_POST['sel'][$i]."'");
		$row=mysql_fetch_array($res);
		$renglon=array();
		if($row['estatus']=='C'){
			$row['cve'].='(C)';
...
if($_POST['ajax']==1) {
	$nivelUsuario = nivelUsuario();
		//Listado de plazas
		$select= " SELECT * FROM recibos_salidas WHERE fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."'";
		if($_POST['fecha_apli_ini'] and $_POST['fecha_apli_fin']){$select.=" AND fecha_aplicacion BETWEEN '".$_POST['fecha_apli_ini']."' AND '".$_POST['fecha_apli_fin']."'";}
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			while($row=mysql_fetch_array($res)) {
				...
				echo '<td align="center">';if($row["estatus"]!="C" and nivelUsuario()>2 and $row["estatus2"]=="1" ){echo'<a href="#" onClick="atcr(\'recibos_salidas.php\',\'\',4,'.$row['cve'].')"><img src="images/validosi.gif" border="0"></a>';}echo''.$array_estatus2[$row["estatus2"]].'</td>';
...
top($_SESSION);
if ($_POST['cmd']==4) {
	$delete= "UPDATE recibos_salidas SET estatus2='2' WHERE cve='".$_POST['reg']."' ";
	$ejecutar=mysql_query($delete);
	$_POST['cmd']=0;
}
if ($_POST['cmd']==3) {
	$delete= "UPDATE recibos_salidas SET estatus='C',fechacan='".fechaLocal()." ".horaLocal()."',usucan='".$_POST['cveusuario']."' WHERE cve='".$_POST['reg']."' ";
	$ejecutar=mysql_query($delete);
	$_POST['cmd']=0;
}
...
if ($_POST['cmd']==2) {
		$insert = " INSERT recibos_salidas 
					SET monto='".$_POST['monto']."',beneficiario='".$_POST['beneficiario']."',concepto='".$_POST['concepto']."',estatus='A',estatus2='1',
						...
						cargo='".$_POST['cargo']."',empresa='".$_POST['empresa']."',fecha_aplicacion='".$_POST['fecha_aplicacion']."',
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		$select=" SELECT * FROM recibos_salidas WHERE cve='".$_POST['reg']."' ";
		$res=mysql_query($select);
		$row=mysql_fetch_array($res);
		if($_POST['reg']>0){
			$fecha=$row['fecha'];
			...
			$fecha=fechaLocal();
			$fechapag=fechaLocal();
			$Encabezado = 'Nuevo Recibo de Salida';
		}
		//Menu
		echo '<table>';
		echo '
			...
			if(nivelUsuario()>1)
				...
				if(document.forma.monto.value==\'\'){
					...
					atcr(\'recibos_salidas.php\',\'\',\'2\',\''.$row['cve'].'\');"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
			echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'recibos_salidas.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>
			</tr>';
		echo '</table>';
		echo '<br>';
		...
		//Formulario 
		echo '<table>';
		echo '<tr><td class="tableEnc">Edicion de Recibos de Salida</td></tr>';
		echo '</table>';
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<td><a href="#" onClick="atcr(\'recibos_salidas.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>
				<td><a href="#" onClick="atcr(\'recibos_salidas.php\',\'_blank\',\'101\',\'0\');"><img src="images/b_print.png" border="0"></a>&nbsp;Imprimir</td><td>&nbsp;</td>
				<td><a href="#" onClick="atcr(\'recibos_salidas.php\',\'_blank\',\'102\',\'0\');"><img src="images/b_print.png" border="0"></a>&nbsp;Imprimi por Recibos</td><td>&nbsp;</td>
			 </tr>';
		echo '</table>';
		echo '<table>';
		echo '<tr>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.send("ajax=1&empresa="+document.getElementById("empresa").value+"&cargo="+document.getElementById("cargo").value+"&fecha_ini="+document.getElementById("fecha_ini").value+"&fecha_fin="+document.getElementById("fecha_fin").value+"&fecha_apli_ini="+document.getElementById("fecha_apli_ini").value+"&fecha_apli_fin="+document.getElementById("fecha_apli_fin").value+"&motivo="+document.getElementById("motivo").value+"&beneficiario="+benef+"&numeroPagina="+document.getElementById("numeroPagina").value+"&usuario="+document.getElementById("usuario").value+"&cveusuario="+document.getElementById("cveusuario").value+"&cvemenu="+document.getElementById("cvemenu").value);
