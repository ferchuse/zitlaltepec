...
if($_POST['ajax']==1){
	...
	echo '<th>Fecha Movimiento</th><th>Usuario</th><th>Importe</th><th>Beneficiario</th>
	</tr>'; 
	$x=0;
	$filtroruta="";
	$select="SELECT a.* FROM pagotaquillatijera a ";
	...
	while($row=mysql_fetch_array($res)){
		...
		if($row['estatus']!='C'){
			echo '<a href="#" onClick="atcr(\'pagotaquillatijera.php\',\'_blank\',10,\''.$row['cve'].'\')"><img src="images/b_print.png" border="0" title="Imprimir"></a>';
			if($nivelUsuario > 2)
				echo '&nbsp;&nbsp;<a href="#" onClick="if(confirm(\'Esta seguro de cancelar el registro?\')) atcr(\'pagotaquillatijera.php\',\'\',3,\''.$row['cve'].'\')"><img src="images/validono.gif" border="0" title="Cancelar"></a>';
		}
		...
		echo '<td align="center">'.utf8_encode($row['beneficiario']).'</td>';
		echo '</tr>';
		$x++;
	}
...
if($_POST['cmd']==10){
	$res = mysql_query("SELECT * FROM pagotaquillatijera WHERE cve='".$_POST['reg']."'");
	$row = mysql_fetch_array($res);
	$texto =chr(27)."@".'|';
	$texto.=chr(27).'!'.chr(10)."PAGO TAQUILLA TIJERA";
	$texto.='||';
	...
	$texto.=sprintf("%-10s","");
	$texto.='|';
	$res1=mysql_query("SELECT * FROM pagotaquillatijeradetalle WHERE pago='".$_POST['reg']."'");
	...
	$texto.='|';
	$texto.=chr(27).'!'.chr(10)."BENEFICIARIO:| ".$row['beneficiario'];
	$texto.='|';
...
if($_POST['ajax']==2){
	...
	if($resultado==0){
		foreach($datos['cantidad'] as $k=>$v){
			if($v>0){
				$res = mysql_query("SELECT a.cve FROM pagotaquillatijera a INNER JOIN pagotaquillatijeradetalle b ON a.cve = b.pago WHERE a.estatus!='C' AND (b.folioini BETWEEN '".$datos['folioini'][$k]."' AND '".$datos['foliofin'][$k]."' OR b.foliofin BETWEEN '".$datos['folioini'][$k]."' AND '".$datos['foliofin'][$k]."'");
...
if($_POST['cmd']==3){
	mysql_query("UPDATE pagotaquillatijera SET estatus='C',fechacan='".fechaLocal()." ".horaLocal()."',usucan='".$_POST['cveusuario']."' WHERE cve='".$_POST['reg']."'");
	$_POST['cmd']=0;
}

if($_POST['cmd']==2){
	mysql_query("INSERT pagotaquillatijera SET fecha_aplicacion='".$_POST['fecha_aplicacion']."',monto='".$_POST['monto']."',
	...
	foreach($_POST['cantidad'] as $k=>$v){
		if($v>0){
			mysql_query("INSERT pagotaquillatijeradetalle SET pago='$cverecaudacion',folioini='".$_POST['folioini'][$k]."',foliofin='".$_POST['foliofin'][$k]."',cantidad='$v',monto='".$_POST['montob'][$k]."'");
		}
	}
	$res = mysql_query("SELECT * FROM pagotaquillatijera WHERE cve='".$cverecaudacion."'");
	$row = mysql_fetch_array($res);
	$texto =chr(27)."@".'|';
	$texto.=chr(27).'!'.chr(10)."PAGO TAQUILLA TIJERA";
	$texto.='||';
	...
	$texto.=sprintf("%-10s","");
	$texto.='|';
	$res1=mysql_query("SELECT * FROM pagotaquillatijeradetalle WHERE pago='".$cverecaudacion."'");
	while($row1=mysql_fetch_array($res1)){
		$texto.=sprintf("%-10s",$row1['folioini']);
		$texto.=sprintf("%-10s",$row1['foliofin']);
		$texto.=sprintf("%-10s",$row1['cantidad']);
...
if($_POST['cmd']==1){
	...
	if(nivelUsuario()>1 && $_POST['reg']==0)
		echo '<td><a href="#" onClick="
				if(!validarfolios()){
					...
					atcr(\'pagotaquillatijera.php\',\'\',\'2\',\''.$_POST['reg'].'\');
				}
				"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
	echo '<td><a href="#" onClick="atcr(\'pagotaquillatijera.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>
			</tr>';
	echo '</table>';
	echo '<br>';
	echo '<table>';
	...
	echo '<tr><th align="left">Beneficiario</td><td><input type="text" class="textField" size="50" name="beneficiario" id="beneficiario" value=""></td></tr>';
...
if($_POST['cmd']==0){
	...
	echo '<tr>
			...
			<td><a href="#" onClick="atcr(\'pagotaquillatijera.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>
		</tr>';
	echo '</table>';
	echo '<table>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","pagotaquillatijera.php",true);
