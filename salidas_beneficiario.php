...
if($_POST['ajax']==1){
	$filtro="";
	$select="SELECT a.beneficiario,sum(a.monto) as importe
	FROM recibos_salidas as a
	INNER JOIN beneficiarios_salidas as b on (b.cve=a.beneficiario) WHERE a.estatus='A'";
...
//	if ($_POST['plaza']!="all") {$select.=" AND a.plaza='".$_POST['plaza']."'";}
	$select.=" GROUP BY a.beneficiario ORDER BY b.nombre";
	...
	if($_SESSION['PlazaUsuario']==0)
		echo '<th>Plaza</th>';
	echo '<th>Beneficiario</th><th>Tipo</th><th>Importe</th>';
	echo '</tr>'; 
	$x=0;
	$total=0;
	while ($Motivos=mysql_fetch_array($rsmotivos)) {
		rowb();
		echo '<td align="center" width="40" nowrap><a href="#" onClick="atcr(\'salidas_beneficiario.php\',\'\',\'1\',\''.$Motivos['beneficiario'].'|'.$Motivos['tipo_beneficiario'].'\')"><img src="images/b_search.png" border="0" title="Ver"></a></td>';
		if($_SESSION['PlazaUsuario']==0)
			echo '<td>'.htmlentities($array_plaza[$Motivos['plaza']]).'</td>';
		echo '<td align="left">'.htmlentities(utf8_encode($array_benef[$Motivos['beneficiario']])).'</td>';
...
if($_POST['cmd']==1){
	...
	echo '<table>';
	echo '<tr>
			<td><a href="#" onclick="atcr(\'salidas_beneficiario.php\',\'\',0,\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;&nbsp;Regresar</a></td>
		</tr>';
	echo '</table>';
	echo '<br>';
	...
	$beneficiario="";
//	if($datos[1]==0)
		$beneficiario=$array_benef[$datos[0]];
//	elseif($datos[1]==1)
//		$beneficiario=$array_unidad[$datos[0]];
//	elseif($datos[1]==2)
//		$beneficiario=$array_conductor[$datos[0]];
	$tit="";
	$select="SELECT * FROM recibos_salidas WHERE  beneficiario='".$datos[0]."'";
...
//	if ($_POST['searchplaza']!="all") {$select.=" AND plaza='".$_POST['searchplaza']."'";}
	...
	$rsmotivos=mysql_db_query($base,$select) or die(mysql_error());
	echo '<table>';
	echo '<tr><td class="tableEnc">Salidas por Motivo '.$beneficiario.$tit.'</td></tr>';
	echo '</table>';
	echo '<br>';
	echo '<table width="100%" border="0" cellpadding="4" cellspacing="1" class="">';
	...
	echo '<th>Folio</th><th>Fecha</th><th>Monto</th><th>Motivo</th><th>Estatus</th><th>Usuario</th>';
	...
	while($Salida=mysql_fetch_array($rsmotivos)) {
		...
		echo '<td align="left">'.htmlentities($array_motivo[$Salida['motivo']]).'</td>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","salidas_beneficiario.php",true);
