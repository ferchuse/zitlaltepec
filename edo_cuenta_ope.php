...
if($_POST['cmd']==100){
	...
	class FPDF2 extends PDF_MC_Table {
		function Header(){
			...
			//$this->Ln();
			$this->SetY(23);
			$this->MultiCell(275,5,'Estado de Cuenta de Operadores del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'],0,'C');
	...
	for($i=0;$i<count($_POST['operadores']);$i++){
		...
		$saldoanterior=saldo_operador($row['cve'],1,0,$_POST['fecha_ini'],$_POST['fecha_fin']);
		$renglon=array();
		$cargo=saldo_operador($row['cve'],2,1,$_POST['fecha_ini'],$_POST['fecha_fin']);
		$abono=saldo_operador($row['cve'],2,2,$_POST['fecha_ini'],$_POST['fecha_fin']);
		$renglon[]=$row['cve'];
		$renglon[]=$row['nombre'];
...
if($_POST['cmd']==101){
	...
	class FPDF2 extends PDF_MC_Table {
		function Header(){
			...
			//$this->Ln();
			$this->SetY(23);
			$this->MultiCell(190,5,'Estado de Cuenta de Operadores del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'],0,'C');
	...
	for($i=0;$i<count($_POST['operadores']);$i++){
		...
		$x=0;
		$saldo=saldo_operador($row['cve'],1,0,$_POST['fecha_ini'],$_POST['fecha_fin']);
		$renglon=array();
		$renglon[]=$fecha;
		...
		for($j=1;$fecha<=$_POST['fecha_fin'];$j++){
			$res1=mysql_query("SELECT * FROM deuda_operador_cargo WHERE estatus!='C' AND fecha_aplicacion='$fecha' AND operador='".$row['cve']."'");
			while($row1=mysql_fetch_array($res1)){
				$sumacargo+=$row1['monto'];
				$saldo-=$row1['monto'];
				$renglon=array();
				$renglon[]=$fecha;
				$renglon[]='Cargo a operador #'.$row1['cve'].' Unidad: '.$array_unidad[$row1['unidad']];
			...
			$res1=mysql_query("SELECT * FROM deuda_operador_abono WHERE estatus!='C' AND fecha_aplicacion='$fecha' AND operador='".$row['cve']."'");
			while($row1=mysql_fetch_array($res1)){
...
header("Expires: 0");
	...
	$row=mysql_fetch_array($res);
	echo '<table>';
	echo '<tr><td class="tableEnc">Estado de Cuenta del Operador ('.$row['cve'].') '.$row['nombre'].'<br>
	...
	for($i=1;$fecha<=$_POST['fecha_fin'];$i++){
		$res1=mysql_query("SELECT * FROM deuda_operador_cargo WHERE fecha_aplicacion='$fecha' AND operador='".$row['cve']."' AND estatus!='C'");
		while($row1=mysql_fetch_array($res1)){
			$sumacargo+=$row1['monto'];
			$saldo-=$row1['monto'];
			//	rowb();
			echo'<tr>';
			echo '<td align=center>&nbsp;'.$fecha.'</td>';
			echo '<td align=left>Cargo a operador #'.$row1['cve'].' Unidad: '.$array_unidad[$row1['unidad']].'</td>';
		...
		$res1=mysql_query("SELECT * FROM deuda_operador_abono WHERE fecha_aplicacion='$fecha' AND operador='".$row['cve']."' AND estatus!='C'");
		while($row1=mysql_fetch_array($res1)){
			$sumaabono+=$row1['monto'];
			$saldo+=$row1['monto'];
			//	rowb();
			echo'<tr>';
			echo '<td align=center>&nbsp;'.$fecha.'</td>';
			echo '<td align=left>Abono a operador #'.$row1['cve'].' Unidad: '.$array_unidad[$row1['unidad']].'</td>';
			echo '<td align="right">'.number_format(0,2).'</td>';
			echo '<td align="right">'.number_format($row1['monto'],2).'</td>';
			echo '<td align="right">'.number_format($saldo,2).'</td>';
			echo '<td align=left>'.$row1['obs'].'&nbsp;</td>';
...
if($_POST['cmd']==1){
	...
	echo '<tr>
			...
			<td><a href="#" onclick="atcr(\'edo_cuenta_ope.php\',\'\',0,\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;&nbsp;Regresar</a></td>
	...
	function buscar_cargos(operador)
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","edo_cuenta_ope.php",true);
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<td><a href="#" onclick="atcr(\'edo_cuenta_ope.php\',\'_blank\',\'100\',\'0\');"><img src="images/b_print.png" border="0">&nbsp;&nbsp;Imprimir</a></td>
				<td><a href="#" onclick="atcr(\'edo_cuenta_ope.php\',\'_blank\',\'101\',\'0\');"><img src="images/b_print.png" border="0">&nbsp;&nbsp;Imprimir Detallados</a></td>
				</tr>';
		echo '</table>';
		echo '<table>';
