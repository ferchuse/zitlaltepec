...
if($_POST['cmd']==100){
	...
	$rssalida=mysql_query("SELECT * FROM recibos_salidas_accidentes WHERE cve='".$_POST['reg']."'");
	...
	$pdf->Cell(95,10,'Recibo de Salida de Accidente',0,0,'L');
...
if($_POST['cmd']==101){
	...
	class FPDF2 extends PDF_MC_Table {
		function Header(){
			...
			$this->SetY(23);
			$this->MultiCell(275,5,'Listado de Recibos de Salida de Accidente',0,'C');
			$this->Ln();
	...
	for($i=0;$i<count($_POST['sel']);$i++){
		$res=mysql_query("SELECT * FROM recibos_salidas_accidentes WHERE cve='".$_POST['sel'][$i]."'");
		$row=mysql_fetch_array($res);
		$renglon=array();
		if($row['estatus']=='C'){
			$row['cve'].='(C)';
...
if($_POST['ajax']==1) {
		//Listado de plazas
		$select= " SELECT a.* FROM recibos_salidas_accidentes a 
...
if ($_POST['cmd']==3) {
	$delete= "UPDATE recibos_salidas_accidentes SET estatus='C',fechacan='".fechaLocal()." ".horaLocal()."',usucan='".$_POST['cveusuario']."' WHERE cve='".$_POST['reg']."' ";
...
if ($_POST['cmd']==2) {
		$insert = " INSERT recibos_salidas_accidentes 
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		$select=" SELECT * FROM recibos_salidas_accidentes WHERE cve='".$_POST['reg']."' ";
		$res=mysql_query($select);
		$row=mysql_fetch_array($res);
		if($_POST['reg']>0){
			$fecha=$row['fecha'];
		...
		echo '
			...
			if(nivelUsuario()>1)
				...
				/*else if(document.forma.recaudacion.value==\'0\'){
					...
					atcr(\'recibos_salidas_accidentes.php\',\'\',\'2\',\''.$row['cve'].'\');"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
			echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'recibos_salidas_accidentes.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>
			</tr>';
		echo '</table>';
		echo '<br>';
		...
		//Formulario 
		echo '<table>';
		echo '<tr><td class="tableEnc">Edicion de Recibos de Salida de Accidente</td></tr>';
		echo '</table>';
		...
		echo '<script>
				function traeUni(){
					...
					if (objeto1.readyState != 0) {
						...
						objeto1.open("POST","recibos_salidas_accidentes.php",true);
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<td><a href="#" onClick="atcr(\'recibos_salidas_accidentes.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>
				<td><a href="#" onClick="atcr(\'recibos_salidas_accidentes.php\',\'_blank\',\'101\',\'0\');"><img src="images/b_print.png" border="0"></a>&nbsp;Imprimir</td><td>&nbsp;</td>
			 </tr>';
		echo '</table>';
		echo '<table>';
		echo '<tr>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","recibos_salidas_accidentes.php",true);
