...
$array_modulos=array(1=>"Catalogos", 2=>"Parque Vehicular",3=>"Operadores",4=>'Recaudacion Unidades',
	6=>'Recaudacion Operadores',5=>'Movimientos',7=>'Accidentes',99=>"Administracion");
//
	...
	if($_POST['plazausuario']>0){
		...
		foreach($array_modulos as $k=>$v){ 
			...
			elseif($SESSION['TipoUsuario']==1){
				...
				$rs=mysql_query("SELECT a.* FROM menu as a INNER JOIN usuario_accesos as b ON (b.menu=a.cve AND b.usuario='".$_POST['cveusuario']."' AND b.plaza='".$_POST['plazausuario']."' AND b.acceso>0) WHERE a.modulo='$k' ORDER BY a.orden");
			}
	...
	function saldo_unidad($unidad, $tipo = 0, $dato = 0, $fecha_ini = "", $fecha_fin = ""){
		...
		if($tipo == 0){
			if($dato == 0 || $dato == 1){
				...
				$row = mysql_fetch_array($res);
				$cargo += $row[0];
				$res = mysql_query("SELECT SUM(total) FROM cargos_variables_unidadeszitla WHERE unidad = '$unidad' AND estatus!='C' AND fecha BETWEEN '$fecha_ini' AND '$fecha_fin'");
				$row = mysql_fetch_array($res);
				$cargo += $row[0];
