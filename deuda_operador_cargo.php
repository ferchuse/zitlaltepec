...
if($_POST['ajax']==1){
	...
	$x=0;
	$filtroruta="";
	$select="SELECT a.* FROM deuda_operador_cargo a ";
	...
	while($row=mysql_fetch_array($res)){
		...
		if($row['estatus']!='C'){
			echo '<a href="#" onClick="atcr(\'deuda_operador_cargo.php\',\'_blank\',10,\''.$row['cve'].'\')"><img src="images/b_print.png" border="0" title="Imprimir"></a>';
			if($nivelUsuario > 2)
				echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'deuda_operador_cargo.php\',\'\',3,\''.$row['cve'].'\')"><img src="images/validono.gif" border="0" title="Cancelar"></a>';
		}
		...
		$total_=$total_ + $row['monto']*$factor;
...
//	foreach($totales as $total)
		echo '<th align="right">'.number_format($total_,2).'</th>';
...
if($_POST['cmd']==10){
	$res = mysql_query("SELECT * FROM deuda_operador_cargo WHERE cve='".$_POST['reg']."'");
	$row = mysql_fetch_array($res);
	$texto =chr(27)."@".'|';
	$texto.=chr(27).'!'.chr(40)."  CARGO DEUDA OPERADOR";
	$texto.='||';
...
if($_POST['cmd']==3){
	mysql_query("UPDATE deuda_operador_cargo SET estatus='C',fechacan='".fechaLocal()." ".horaLocal()."',usucan='".$_POST['cveusuario']."' WHERE cve='".$_POST['reg']."'");
	$_POST['cmd']=0;
}

if($_POST['cmd']==2){
		mysql_query("INSERT deuda_operador_cargo SET fecha_aplicacion='".$_POST['fecha_aplicacion']."',
		...
		$res = mysql_query("SELECT * FROM deuda_operador_cargo WHERE cve='".$cverecaudacion."'");
		$row = mysql_fetch_array($res);
		$texto =chr(27)."@".'|';
		$texto.=chr(27).'!'.chr(40)."  CARGO DEUDA OPERADOR";
		$texto.='||';
	...
	if($nivelUsuario>1)
		echo '<td><a href="#" onClick="
				if(document.forma.recaudacion.value==\'0\'){
					...
					atcr(\'deuda_operador_cargo.php\',\'\',\'2\',\'0\');
				}
				"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
		echo '<td><a href="#" onClick="atcr(\'deuda_operador_cargo.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>';
	
	echo '</tr>';
	echo '</table>';
	echo '<br>';
	echo '<table>';
	echo '<tr><td class="tableEnc">Cargo Deuda Operador</td></tr>';
	echo '</table>';
	echo '<table>';
...
if($_POST['cmd']==0){
	...
	echo '<tr>
			...
			<td><a href="#" onClick="atcr(\'deuda_operador_cargo.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>
		</tr>';
	echo '</table>';
	echo '<table>';
	$dia=date("w");
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","deuda_operador_cargo.php",true);
