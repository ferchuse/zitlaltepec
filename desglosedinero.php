<?php
	...
	{	
		...
		//Si el permiso de usuario en esta pagina es diferente a supervisor (4) clase = readOnly
		
		
		$consulta_permiso = "SELECT * FROM usuario_accesos LEFT JOIN menu on menu.cve = usuario_accesos.menu
		
		WHERE usuario = {$_SESSION["CveUsuario"]} AND link = 'desglosedinero.php'";
		
		$result_permiso =mysql_db_query($base,$consulta_permiso)or die(mysql_error());
		
		while ($fila=mysql_fetch_array($result_permiso)){
		...
		if($fila_permiso["acceso"] == "3"){
			$readonly = "";
		}
		
		echo "ACCESO". $fila_permiso["acceso"];
		
		echo '<tr><th align="left">Fecha</th><td><input type="date" name="fechahoy" id="fechahoy" class="'.$readonly.'" size="15" value="'.$row['fecha'].'"'.$readonly.'>';
		if($_SESSION[$archivo[(count($archivo)-1)]]>2)
